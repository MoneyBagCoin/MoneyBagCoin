...
void Shutdown()
{
    ...
    RenameThread("moneybagcoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: MoneyBagCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: moneybagcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMoneyBagCoinamount=<n> " + _("Keep N MoneyBagCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MoneyBagCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MoneyBagCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MoneyBagCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "MoneyBagCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MoneyBagCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart MoneyBagCoin to complete") << "\n";
    ...
    nAnonymizeMoneyBagCoinAmount = GetArg("-anonymizeMoneyBagCoinamount", 0);
    if(nAnonymizeMoneyBagCoinAmount > 999999) nAnonymizeMoneyBagCoinAmount = 999999;
    if(nAnonymizeMoneyBagCoinAmount < 2) nAnonymizeMoneyBagCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeMoneyBagCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize MoneyBagCoin Amount %d\n", nAnonymizeMoneyBagCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MoneyBagCoin+1000 == (.1MoneyBagCoin+100)*10
       10MoneyBagCoin+10000 == (1MoneyBagCoin+1000)*10
    */
