...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("MoneyBagCoin"))
        return false;
...
{
    // Convert MoneyBagCoin:// to MoneyBagCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("MoneyBagCoin://"))
    {
        uri.replace(0, 12, "MoneyBagCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MoneyBagCoin.lnk";
}
...
{
    return GetAutostartDir() / "MoneyBagCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MoneyBagCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  MoneyBagCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MoneyBagCoin-Qt"));
