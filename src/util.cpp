...
int nAnonymizeMoneyBagCoinAmount = 500;
...
#else
    const char* pszModule = "MoneyBagCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MoneyBagCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MoneyBagCoin
    // Mac: ~/Library/Application Support/MoneyBagCoin
    // Unix: ~/.MoneyBagCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MoneyBagCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "MoneyBagCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "moneybagcoind.pid"));
